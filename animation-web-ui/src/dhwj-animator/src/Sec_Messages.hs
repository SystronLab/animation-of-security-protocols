...
expGLst ::
  forall a b c d e f g.
    (Type_Length.Len a, Type_Length.Len b, Type_Length.Len c, Type_Length.Len d,
      Type_Length.Len e, Type_Length.Len f,
      Type_Length.Len g) => [FSNat.Fsnat a] -> [Dmsg b c d e a f g];
expGLst xs = map MExpg xs;
...
allExpGs :: forall a. (Type_Length.Len a) => [FSNat.Fsnat a];
allExpGs = FSNat.enum_fsnat;
...
is_cjam_C (Cdejam_C x4) = False;
is_cjam_C (Recv_C x5) = False;
is_cjam_C (Leak_C x6) = False;
is_cjam_C (Sig_C x7) = False;
...
swap_mod_exp ::
  forall a b c d e f g.
    (Type_Length.Len a, Type_Length.Len b, Type_Length.Len c, Type_Length.Len d,
      Type_Length.Len e, Type_Length.Len f,
      Type_Length.Len g) => Dmsg a b c d e f g -> Dmsg a b c d e f g;
swap_mod_exp (MModExp (MModExp (MExpg x) a) b) =
  MModExp (MModExp (MExpg x) b) a;
swap_mod_exp (MAg v) = MAg v;
swap_mod_exp (MNon v) = MNon v;
swap_mod_exp (MK v) = MK v;
swap_mod_exp (MPair v va) = MPair v va;
swap_mod_exp (MAEnc v va) = MAEnc v va;
swap_mod_exp (MSig v va) = MSig v va;
swap_mod_exp (MSEnc v va) = MSEnc v va;
swap_mod_exp (MExpg v) = MExpg v;
swap_mod_exp (MModExp (MAg vb) va) = MModExp (MAg vb) va;
swap_mod_exp (MModExp (MNon vb) va) = MModExp (MNon vb) va;
swap_mod_exp (MModExp (MK vb) va) = MModExp (MK vb) va;
swap_mod_exp (MModExp (MPair vb vc) va) = MModExp (MPair vb vc) va;
swap_mod_exp (MModExp (MAEnc vb vc) va) = MModExp (MAEnc vb vc) va;
swap_mod_exp (MModExp (MSig vb vc) va) = MModExp (MSig vb vc) va;
swap_mod_exp (MModExp (MSEnc vb vc) va) = MModExp (MSEnc vb vc) va;
swap_mod_exp (MModExp (MExpg vb) va) = MModExp (MExpg vb) va;
swap_mod_exp (MModExp (MModExp (MAg vd) vc) va) =
  MModExp (MModExp (MAg vd) vc) va;
swap_mod_exp (MModExp (MModExp (MNon vd) vc) va) =
  MModExp (MModExp (MNon vd) vc) va;
swap_mod_exp (MModExp (MModExp (MK vd) vc) va) =
  MModExp (MModExp (MK vd) vc) va;
swap_mod_exp (MModExp (MModExp (MPair vd ve) vc) va) =
  MModExp (MModExp (MPair vd ve) vc) va;
swap_mod_exp (MModExp (MModExp (MAEnc vd ve) vc) va) =
  MModExp (MModExp (MAEnc vd ve) vc) va;
swap_mod_exp (MModExp (MModExp (MSig vd ve) vc) va) =
  MModExp (MModExp (MSig vd ve) vc) va;
swap_mod_exp (MModExp (MModExp (MSEnc vd ve) vc) va) =
  MModExp (MModExp (MSEnc vd ve) vc) va;
swap_mod_exp (MModExp (MModExp (MModExp vd ve) vc) va) =
  MModExp (MModExp (MModExp vd ve) vc) va;
swap_mod_exp (MModExp (MModExp (MBitm vd) vc) va) =
  MModExp (MModExp (MBitm vd) vc) va;
swap_mod_exp (MModExp (MModExp (MWat vd ve) vc) va) =
  MModExp (MModExp (MWat vd ve) vc) va;
swap_mod_exp (MModExp (MModExp (MJam vd ve) vc) va) =
  MModExp (MModExp (MJam vd ve) vc) va;
swap_mod_exp (MModExp (MBitm vb) va) = MModExp (MBitm vb) va;
swap_mod_exp (MModExp (MWat vb vc) va) = MModExp (MWat vb vc) va;
swap_mod_exp (MModExp (MJam vb vc) va) = MModExp (MJam vb vc) va;
swap_mod_exp (MBitm v) = MBitm v;
swap_mod_exp (MWat v va) = MWat v va;
swap_mod_exp (MJam v va) = MJam v va;
