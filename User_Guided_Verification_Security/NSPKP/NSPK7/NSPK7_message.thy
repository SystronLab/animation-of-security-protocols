...
@{text "ks"}.\<close>
fun break_sk::"dmsg list \<Rightarrow> dkey set \<Rightarrow> dmsg list" where
"break_sk [] sks = []" |      
...
"break_sk ((MSig A (SK k))#xs) sks = 
    (if Kp (PK k) \<in> sks then 
      List.insert (MSig A (SK k)) (remdups (break_sk [A] sks @ break_sk xs sks))
    else 
      List.insert (MSig A (SK k)) (break_sk xs sks))"

value "let knowledge = [
    MNon (N Alice), 
    \<lbrace>{(\<lbrace>(MAg Bob), (MNon (N Alice))\<rbrace>\<^sub>m)}\<^bsub>PK Bob\<^esub> , (MAg Alice)\<rbrace>\<^sub>m,
    MKs (SK Bob)] 
  in break_sk knowledge (set (extract_dkeys knowledge))"
...
definition "breakm xs = break_sk xs (set (extract_dkeys xs))"
...
"build\<^sub>n xs pks sks 0 (Suc ne) l = (build\<^sub>n (List.union (enc\<^sub>1 xs pks) xs) pks sks 0 ne l)" |
...
"build\<^sub>n xs pks sks (Suc nc) (Suc ne) l = (if ne = 0 then
  ...
  \<comment> \<open> New messages after encryption \<close>
    (build\<^sub>n (List.union (enc\<^sub>1 xs pks) xs) pks sks (Suc nc) ne l)
  )
)"
...
definition "build\<^sub>n_1 ns = (build\<^sub>n ns (extract_dpkey ns) (extract_dskey ns) 1 1 3)"

definition "build\<^sub>n_2 ns = (build\<^sub>n ns (extract_dpkey ns) (extract_dskey ns) 2 1 3)"
